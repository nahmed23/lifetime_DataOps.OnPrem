...
CREATE         PROCEDURE [dbo].[mmsPrepareDSSRCashReductionSummary]
AS
BEGIN
SET XACT_ABORT ON
SET NOCOUNT ON

  TRUNCATE TABLE DSSRCashReductionSummary
...
-- Report Logging
  ...
  SELECT C.ClubName, C.ClubID, C.ClubActivationDate
   ...
   -----
   -----  This is the query to gather all of the Initiation Fee, Admin.Fee and Membership Add on product sales for the above collected memberships
   -----

   INSERT INTO vDSSRCashReductionSummary(ClubID,ClubName,MembershipID,EventDate,EventDescription,Today_Flag,EventTranItemID,EventItemAmount,
                  MMSTranID,MemberID,TranReasonDescription,JoinDate,PostDateTime,CommEmplFirstName,CommEmplLastName,
                  ProductDescription,CommissionCount,TranItemID, ItemAmount,PrimaryFirstName,PrimaryLastName,TranType,
                  CommEmployeeID)
   ...
   AND AL3.ProductID IN (88, 89, 286, 3132) AND AL8.ValMemberTypeID=1) AND
   (T.ClubActivationDate > DateAdd(d, -60, @Yesterday) OR
    T.ClubActivationDate Is Null) --Recently Opened Clubs
UNION
   SELECT DISTINCT CASE 
          WHEN AL10.ClubID IS NULL THEN AL9.ClubID
          ELSE AL10.ClubID END ClubID,
   ...
   FROM dbo.vMMSTran AL1 JOIN dbo.vReasonCode AL2 ON AL1.ReasonCodeID=AL2.ReasonCodeID
                         ...
                         LEFT OUTER JOIN #DownGrade AL9 ON AL1.MembershipID = AL9.MembershipID
                         LEFT OUTER JOIN #Cancellation AL10 ON AL1.MembershipID = AL10.MembershipID
   WHERE (AL1.TranVoidedID IS NULL AND AL1.PostDateTime > (DateAdd(d,-90,@ToDay))
   AND AL3.ProductID IN (88, 89, 286, 3132) AND AL8.ValMemberTypeID=1) AND
   T.ClubActivationDate <= DateAdd(d, -60, @Yesterday) --Clubs Opened before 60 days
