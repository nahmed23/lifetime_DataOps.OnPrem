...
CREATE           PROC dbo.mmsRevenuerpt_Revenue_adhoc (
...
-- Report Logging
  ...
  BEGIN
          SELECT PostingClubName, ItemAmount, DeptDescription, ProductDescription, MembershipClubname,
                 ...
                 MembershipID, ProductID, TranClubid, Quantity, @StartPostDate AS ReportStartDate,
  ...
  BEGIN
          ...
          WHERE MMST.PostDateTime >= @StartPostDate AND
                ...
                C2.DisplayUIFlag = 1
                AND VR.ValRegionID = 1  ----- ad hoc line added
	  ...
	  SELECT C2.ClubName PostingClubName, TI.ItemAmount, D.Description DeptDescription,
	         ...
	         P.ProductID, MMST.ClubID TranClubid, TI.Quantity,@StartPostDate AS ReportStartDate,@EndPostDate AS ReportEndDate
	  FROM dbo.vMMSTran MMST
	       JOIN dbo.vClub C
