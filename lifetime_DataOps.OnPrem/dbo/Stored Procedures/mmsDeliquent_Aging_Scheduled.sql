...
CREATE         PROC [dbo].[mmsDeliquent_Aging_Scheduled](
...
--
-- returns all deliquent accounts for the company 
...
SELECT C.ClubName, VR.Description AS RegionDescription, P.Description AS MemberShipTypeDescription,
       ...
       TB.TranItemID, MMST.TranDate as TranDate_Sort,
...
/***************************************/
  ...
  LEFT OUTER JOIN dbo.vMMSTranNonArchive MMST 
...
/*******************************************/
 WHERE (MB.EFTAmount >= 15 OR (MB.EFTAmount >= CP.Price and MB.EFTAmount <>0)) AND
       M.ValMemberTypeID = 1 AND
       TI.TranItemID IS Not Null AND
       DateDiff(Day,MMST.TranDate,GetDate()) <= 90
