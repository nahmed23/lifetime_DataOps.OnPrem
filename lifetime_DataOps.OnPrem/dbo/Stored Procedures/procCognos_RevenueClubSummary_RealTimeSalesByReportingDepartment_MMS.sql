...
CREATE PROC [dbo].[procCognos_RevenueClubSummary_RealTimeSalesByReportingDepartment_MMS] (
    @StartDate DATETIME,
    @EndDate DATETIME,
...
SET @IncludeTodaysTransactionsFlag = CASE WHEN @StartDate >= CONVERT(Datetime,CONVERT(Varchar,GetDate(),101),101) 
                                               ...
                                               THEN 'Y'
                                          ELSE 'N' END
SET @HeaderDateRange = Replace(Substring(convert(varchar, @StartDate, 100),1,6)+', '+Substring(convert(varchar, @StartDate, 100),8,10)+' '+Substring(convert(varchar,@StartDate, 100),18,2),'  ',' ')
                       + ' through ' + 
                       Replace(Substring(convert(varchar, @EndDate, 100),1,6)+', '+Substring(convert(varchar, @EndDate,100),8,10)+' '+Substring(convert(varchar,@EndDate,100),18,2),'  ',' ')
...
SELECT DISTINCT ReportDimReportingHierarchy.DimReportingHierarchyKey,
  ...
  JOIN fnParsePipeList(@DepartmentMinDimReportingHierarchyKeyList) KeyList
    ...
    OR KeyList.Item = 'All Departments'
