...
CREATE PROC [dbo].[procWorkday_EFTSummary] (
...
'American Express|Commercial Checking EFT|Discover|Individual Checking|MasterCard|Savings Account|VISA'
    ...
    =============================================		*/
    DECLARE @ClubIDList VARCHAR(2000) = 0,
    @PaymentTypeList VARCHAR(1000) = 'American Express|Commercial Checking EFT|Discover|Individual Checking|MasterCard|Savings Account|VISA'
    ...
    FROM vClub C
        JOIN  #tmpList tC
            ON tC.StringField = C.ClubID OR tC.StringField = '0'
...
/***************************************/
    SELECT
        C.WorkdayRegion,
        CAST(EFT.EFTDate as date) AS EFTDate,
        ...
        SUM(CASE WHEN EFT.Valeftstatusid = 3 THEN EFT.EFTAmount + IsNull(EFT.EFTAmountProducts,0) ELSE 0 END) AS ApprovedAmount,
        SUM(CASE WHEN EFT.Valeftstatusid = 2 THEN 1 ELSE 0 END) AS ReturnedCount,
        SUM(CASE WHEN EFT.Valeftstatusid = 2 THEN EFT.EFTAmount + IsNull(EFT.EFTAmountProducts,0) ELSE 0 END) AS  ReturnedAmount,
        CASE WHEN VPT.ValPaymentTypeID IN (9,10,13) THEN 'ACH'
            WHEN VPT.ValPaymentTypeID IN (3,4) THEN 'VISA_MC'
            WHEN VPT.ValPaymentTypeID IN (5) THEN 'DISCOVER'
            WHEN VPT.ValPaymentTypeID IN (8) THEN 'AMEX' END AS TenderType,
        MAX(@HeaderDateRange) AS HeaderDateRange,
        MAX(@ReportRunDateTime) AS ReportRunDateTime,
        VCC.CurrencyCode AS CurrencyCode,
        /*SUM(CASE WHEN VPT.ValPaymentTypeID in (9,10,13) AND EFT.Valeftstatusid = 3
                 THEN EFT.EFTAmount else 0 END) Accum_ACHApprovedAmt_ByClub, -- ('Individual Checking', 'Savings Account', 'Commercial Checking EFT')
        SUM(CASE WHEN VPT.ValPaymentTypeID in (3,4) AND EFT.Valeftstatusid = 3
                 THEN EFT.EFTAmount else 0 END) Accum_VISA_MC_ApprovedAmt_ByClub, -- ('VISA', 'MasterCard')
        SUM(CASE WHEN VPT.ValPaymentTypeID in (5) AND EFT.Valeftstatusid = 3
                 THEN EFT.EFTAmount else 0 END) Accum_Discover_ApprovedAmt_ByClub, -- ('Discover')
        SUM(CASE WHEN VPT.ValPaymentTypeID in (8) AND EFT.Valeftstatusid = 3
                 THEN EFT.EFTAmount else 0 END) Accum_AMEX_ApprovedAmt_ByClub, -- ('American Express')*/
...
/*******************************************/
    WHERE
        C.DisplayUIFlag = 1
        AND CAST(EFT.EFTDate as date) >= @StartDate
        -- Through Midnight of the previous day
        AND CAST(EFT.EFTDate as date) < @EndDate
        AND VPT.ViewBankAccountTypeFlag = 1
        AND EFT.EFTReturnCodeID is not null
        AND EFT.EFTReturnCodeID <> 42
        AND EFT.ValEFTTypeID <> 3 -- refunds are not included
    GROUP BY
        C.WorkdayRegion
        ,VCC.CurrencyCode
        ,CAST(EFT.EFTDate as date)
        --,VPT.ValPaymentTypeID
        ,CASE WHEN VPT.ValPaymentTypeID IN (9,10,13) THEN 'ACH'
            WHEN VPT.ValPaymentTypeID IN (3,4) THEN 'VISA_MC'
            WHEN VPT.ValPaymentTypeID IN (5) THEN 'DISCOVER'
            WHEN VPT.ValPaymentTypeID IN (8) THEN 'AMEX' END
    ORDER BY C.WorkdayRegion, CAST(EFT.EFTDate as date)
