...
-- THIS PROCEDURE WILL POPULATE THE MemberReimbursementHistory TABLE WITH ALL ENROLLED MEMBERS OF THE SPECIFIED PROGRAM
...
CREATE Procedure [dbo].[mmsPrepareReimbursementExportData] (
  @ReimbursementProgramID SMALLINT
)
AS
BEGIN
  SET XACT_ABORT ON
  ...
  CREATE TABLE #TaxPercentage(ClubID INT, ProductID INT, TaxPercentage DECIMAL(4,2), ClubProductTaxRateID INT)
  INSERT INTO #TaxPercentage(ClubID, ProductID, TaxPercentage, ClubProductTaxRateID)
  --Find each individual tax rate for every product
  SELECT CPTR.ClubID, CPTR.ProductID, TR.TaxPercentage, CPTR.ClubProductTaxRateID
  ...
  CREATE TABLE #MemberReimbursementHistory(MemberReimbursementHistoryID INT IDENTITY(1,1), MembershipID INT, MemberID INT,
   ...
   -- POPULATE THE #MemberReimbursementHistory TABLE WITH ALL ENROLLED MEMBERS THAT ARE ELIGIBLE
  ...
  -- Sum the total amount of dues and taxes for each Membership
  SELECT MembershipID, MemberID, ReimbursementProgramID, @UsageFirstOfMonth, EnrollmentDate,
         CAST(Price + SUM(TaxedAmount) AS MONEY), 0, NULL, ClubID, NULL, NULL, 0
  FROM (
	    -- Calculate taxes for each tax rate and Round to 2 decimal places
		SELECT DISTINCT MS.MembershipID, MR.MemberID, MR.ReimbursementProgramID, MR.EnrollmentDate, CP.Price,
			   TP.ClubProductTaxRateID, CAST(((ISNULL(TP.TaxPercentage, 0) / 100) * CP.Price) AS NUMERIC(6,2)) TaxedAmount, MS.ClubID
		   ...
		   WHERE (MR.TerminationDate IS NULL OR MR.TerminationDate > @UsageFirstOfMonth)
			 ...
			 AND MR.ReimbursementProgramID = @ReimbursementProgramID
			 AND MR.EnrollmentDate < DATEADD(m, 1, @UsageFirstOfMonth)
  ) AS Pricing
  GROUP BY MembershipID, MemberID, ReimbursementProgramID, EnrollmentDate, Price, ClubID

   -- GET THE MEMBER USAGE FOR THE ACTIVE MEMBERS, MAX OF ONE USE PER DAY.
