CREATE TABLE [dbo].[MMSRevenueGLPostingSummary] (
    [MMSRevenueGLPostingSummaryID]                 BIGINT          IDENTITY (1, 1) NOT NULL,
    ...
    CONSTRAINT [PK_MMSRevenueGLPostingSummary] PRIMARY KEY NONCLUSTERED ([MMSRevenueGLPostingSummaryID] ASC) WITH (FILLFACTOR = 70)
);


GO
CREATE NONCLUSTERED INDEX [IX_Posting_MMSClubID]
    ON [dbo].[MMSRevenueGLPostingSummary]([Posting_MMSClubID] ASC) WITH (FILLFACTOR = 70);


GO
CREATE NONCLUSTERED INDEX [IX_Reconciliation_PostingWorkdayOffer_PostDateTime_Reconciliat_Adj_WorkdayAccount_Reconciliation_PostingWorkdayCe_Includes]
    ON [dbo].[MMSRevenueGLPostingSummary]([Reconciliation_PostingWorkdayOffering] ASC, [PostDateTime] ASC, [Reconciliation_Adj_WorkdayAccount] ASC, [Reconciliation_PostingWorkdayCostCenter] ASC)
    INCLUDE([MMSRevenueGLPostingSummaryID], [LocalCurrencyGLPostingAmount], [USDGLPostingAmount]) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE);


GO
CREATE NONCLUSTERED INDEX [IX_InsertedDateTime]
    ON [dbo].[MMSRevenueGLPostingSummary]([InsertedDateTime] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE);
